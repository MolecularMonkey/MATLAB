function width = fw20m(x,y)
% function width = fw20m(x,y)
%
% Full-Width at 20% Maximum (FWHM) of the waveform y(x)
% and its polarity.
...
while sign(y(i)-lev20) == sign(y(i-1)-lev20)
    i = i+1;
end                                   %first crossing is between v(i-1) & v(i)
interp = (lev20-y(i-1)) / (y(i)-y(i-1));
tlead = x(i-1) + interp*(x(i)-x(i-1));
i = centerindex+1;                    %start search for next crossing at center
while ((sign(y(i)-lev20) == sign(y(i-1)-lev20)) && (i <= N-1))
    i = i+1;
end
if i ~= N
    Ptype = 1;  
