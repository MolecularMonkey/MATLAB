...
[coeff,Pristine_SubstractedSpectraWvnm,Pristine_SubstractedSpectraWvlg,Exc1,Exc2]=SSaSm(PristineData_RRRefBU,ExcitationWaveLength1,ExcitationWaveLength2,0.47,figgg,figgg+1,1);
      ...
      switch SSSSatisfied
       case 'Yes!'
          ...
          exxctflg=1;
        case 'No?'
          definput{1,1}=num2str(Exc1);
          definput{1,2} = num2str(Exc1);
      end 
end
clear exxctflg SSSSatisfied
[coeff,Trated_SubstractedSpectraWvnm,Trated_SubstractedSpectraWvlg,~,~]=SSaS(FunctionalizedData_RRRefBU,ExcitationWaveLength1,ExcitationWaveLength2,coeff,figgg+2,figgg+3);
     case 'Two 2'
         disp('Since the code has not been developed yet you cannot do that!!!!!')
         return
 end
...
while remainflg
    ...
    definput{1,1}=num2str(Emiss1);
    definput{1,2} = num2str(Emiss2);
end
%% Plotting
    ...
    title({['Exc: ',num2str(Exc1),'nm'],'Experimental Data'});
    xlabel('Wavenumber 1/cm')
...
CutSubstractHistory{1,4}=['Main Spectra with Excitation ',num2str(ExcitationWaveLength1),'nm was selected and excitation profile w/ wavelength ',num2str(ExcitationWaveLength2),'nm was substracted from it after multiplying in coeeficient ',num2str(coeff)];
CutSubstractHistory{2,4}=ExcitationWaveLength1; 
...
%% Saving Part
    ...
    save([PathName,FileName],'CutSubstractHistory')
...
%% Clearing Part 
clear DoUhave answer definput dims dlgtitle Excitation Emission figgg
