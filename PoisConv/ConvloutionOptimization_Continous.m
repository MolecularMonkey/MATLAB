...
ModeledFunction=@(Variables2BOpt,X)PoissionCorrelation2(Variables2BOpt(1),Variables2BOpt(2),X,Intensity);
    Variables2BOpt0(1)=1.0;
    ...
    Fsumsquares = @(Variables2BOpt)sum((ModeledFunction(Variables2BOpt,X) - functionalizedCNTwvn(:,2)).^2);
    ...
    fmincon(Fsumsquares,Variables2BOpt0,A,b,Aeq,beq,lb,ub,nonlcon,opts);
...
ChangeperFunctionalization=xunc(2); 
    ...
    hx = PoissionCorrelation(Spectra,lambda,ChangeperFunctionalization);
    %% Visualization
    [~,I]=max(Intensity);
